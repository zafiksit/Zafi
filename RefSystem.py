...
def settings_kb(top: int) -> InlineKeyboardMarkup:
    ...
    kb.add(InlineKeyboardButton("✍️ Изменить награду", callback_data='ref-edit-prize'))
    kb.add(InlineKeyboardButton(txt, callback_data='ref-edit-top'))
    return kb
...
def select_values() -> InlineKeyboardMarkup:
    ...
    kb.add(*buttons)
    kb.add(InlineKeyboardButton("❌ Закрыть", callback_data='ref-dell'))
    return kb
...
async def ref_top_kb(call: types.CallbackQuery, user: BFGuser):
    ...
    await call.message.edit_text(
        text=msg,
        reply_markup=assets.kb.top(user.id, 'ref'),
...
async def start_module():
    ...
    # тут ничего фонового не нужно, просто заглушка
    return
